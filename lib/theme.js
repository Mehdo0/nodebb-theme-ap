...
library.init = async function (params) {
	...
	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/ap', [], controllers.renderAdminPage);
...
library.defineWidgetAreas = async function (areas) {
	...
	const templates = [
		...
		'login.tpl', 'register.tpl', 'bottom-nav.tpl',
	];
...
library.getThemeConfig = async function (config) {
    ...
    // Force the left sidebar to be open and visible
    config.theme['sidebar-left-open'] = 'on';
    config.theme['sidebar-right-open'] = 'off'; // Close right sidebar
    
    // Force navigation to be visible
    config.openDraftsOnPageLoad = false;
    
    console.log('AP Theme: Config applied', config.theme);
    return config;
};

library.getAdminSettings = async function (hookData) {
	if (hookData.plugin === 'ap') {
		hookData.values = {
			...defaults,
			...hookData.values,
		};
...
library.getNavigation = async function (navigation, callback) {
    // Ensure navigation data is passed to templates
    // This hook ensures our navigation data is available
